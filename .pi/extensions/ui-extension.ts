 ...
 *
 * Provides UI status tools and widgets for the pi CLI terminal interface.
 * Integrates with CodepOS agent teams for real-time status monitoring.
 *
 * Usage:
 *   - pi codepos_ui_status [depth]
 *   - pi codepos_list_themes
 *   - pi codepos_cycle_theme [direction|themeName]
 *
 * @version 1.0.0
 * @author CodepOS Team
 * @license MIT
...
/**
 * Available themes list (12 themes)
 */
...
function renderAgentStatus(ctx: ExtensionContext): string {
	...
	const lines = [];
	lines.push("🟦 CodepOS Agent Status");
	lines.push("");
	...
	lines.push("");
	lines.push(`   Active: ${agents.filter(a => ctx.fs.existsSync(.pi/multi-team/agents/a)).length}/${agents.length}`);
 ...
 */
function renderProgress(ctx: ExtensionContext, progress: number): string {
	const lines = [];
	lines.push("");
...
export default function (pi: ExtensionAPI): void {
	// Register UI status tool
	pi.registerTool({
		name: "codepos_ui_status",
		description: "Get current CodepOS system status with agent availability",
		parameters: {
			type: "object",
			properties: {
				depth: {
	...
	// Register theme list tool
	pi.registerTool({
		name: "codepos_list_themes",
		description: "List all available CodepOS themes",
		parameters: {
			type: "object",
			properties: {
				filter: {
	...
	pi.registerTool({
		...
		execute: async (args, ctx) => {
			...
			if (args.themeName && args.themeName.length > 0) {
				const index = findCurrentIndex(args.themeName);
				currentThemeIndex = index;
				const theme = getThemeList()[index];
				return { message: "Applied theme: " + theme.name };
			}
			...
			if (direction === "random") {
				...
				return { message: "Random theme applied: " + randomTheme.name };
			}
			...
			const theme = getThemeList()[currentThemeIndex];
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		// Register UI widgets
		if (ctx.hasUI) {
			// Agent status widget below editor
	...
	pi.on("key_press", async (event, ctx) => {
		...
		if (key === "ArrowLeft" && event.ctrlKey) {
			const result = "Switched to theme: " + getThemeList()[(currentThemeIndex - 1 + THEMES.length) % THEMES.length].name;
			ctx.ui.showNotification({
				placement: "topRight",
				size: 2,
				content: result
		...
		if (key === "ArrowRight" && event.ctrlKey) {
			currentThemeIndex = (currentThemeIndex + 1) % THEMES.length;
			const result = "Switched to theme: " + getThemeList()[currentThemeIndex].name;
			ctx.ui.showNotification({
				placement: "topRight",
				size: 2,
				content: result
		...
		if (key === " " && event.ctrlKey) {
			const randomIndex = Math.floor(Math.random() * THEMES.length);
			currentThemeIndex = randomIndex;
			const result = "Random theme applied: " + THEMES[randomIndex].name;
			ctx.ui.showNotification({
				placement: "topRight",
				size: 2,
				content: result
